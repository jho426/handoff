...
app.post('/api/analyze-image/claude', upload.single('image'), async (req, res) => {
  try {
    if (!anthropic) {
      return res.status(503).json({ error: 'Claude API is not configured. Please add ANTHROPIC_API_KEY to .env file.' });
    }
...
app.post('/api/analyze-image/openai', upload.single('image'), async (req, res) => {
  try {
    if (!openai) {
      return res.status(503).json({ error: 'OpenAI API is not configured. Please add OPENAI_API_KEY to .env file.' });
    }
